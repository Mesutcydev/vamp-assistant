...
enum RemoteModelCatalog {
    static func reasoningEfforts(
        ...
        case .gemini:
            guard modelID.contains("gemini-2.5") || modelID.contains("gemini-3") else { return [] }
            // Gemini 3.7+ rejects the MINIMAL thinking level with an API
            // validation error, so it is not offered for those models.
            return Self.geminiSupportsMinimalThinking(modelID)
                ? geminiEfforts
    ...
    /// Whether this Gemini model accepts the `MINIMAL` thinking level.
    /// Gemini 3.7 removed it; older 3.x models and 2.5 still accept it.
    static func geminiSupportsMinimalThinking(_ model: String) -> Bool {
        let id = model.lowercased()
        guard let range = id.range(of: #"gemini-3(?:\.(\d+))?"#, options: .regularExpression) else {
            return true
        }
        let token = id[range]
...
struct RemoteEndpoint: Codable, Sendable, Equatable {
    ...
    var effectiveProtocol: RemoteAPIProtocol {
        // Older model discovery stamped every OpenCode model with the
        // selected model's protocol. Resolve the mixed gateway per model,
        // including endpoints restored from those persisted profiles.
        if provider == .openCode || provider == .openCodeGo {
            return provider.remoteAPIProtocol(for: model)
        }
        return apiProtocol ?? provider.remoteAPIProtocol(for: model)
    }
